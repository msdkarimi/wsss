...
class Net(nn.Module):
    ...
    def forward(self, x, valid_mask, name=None, category=None):
        ...
        # norm shape b, c, h, w
        batch_activations = []
        for _img_idx in range(norm_cam.shape[0]):
            activations = torch.zeros_like(valid_mask[_img_idx])
            _mask = norm_cam[_img_idx].detach().clone()
            _mask = (_mask > 0.5).cpu().numpy() * valid_mask[_img_idx].detach().cpu().numpy() # c, h, w
            # print('sdfs')


        # fut_for_mask = torch.sum(norm_cam.detach(), dim=1).cpu().numpy()
        #     activations_of_give_image = []
        # for idx_image in range(norm_cam.shape[0]):
            _init_seed_ = self.get_seeds_from_cam(_mask, category[_img_idx])
            for _present_label in category[_img_idx].numpy().tolist():
                if _present_label == 0:
                    break
                _init_seed = torch.from_numpy(_init_seed_[_present_label])
                num_sp = max(len(torch.nonzero(_init_seed[:, 0])), len(torch.nonzero(_init_seed[:, 1])))

                if (num_sp == 0) or (num_sp == 1):
                    _prototype = self.Weighted_GAP(layer1234_cpfe_feats[_img_idx], _mask[_present_label])  # 1 x c x 1 x 1
                    _activation = self.activate_features(layer1234_cpfe_feats[_img_idx], _prototype)
                    # activations.append(_activation * valid_mask[_img_idx])
                    activations[_present_label] = _activation
                    continue

                s_seed_ = _init_seed[:num_sp, :].cuda()

                _prototypes = self.update_super_pixel_centroid(layer1234_cpfe_feats[_img_idx], _mask[_present_label], s_seed_, 10)
                _activation = self.activate_features(layer1234_cpfe_feats[_img_idx], _prototypes)
                activations[_present_label] = _activation
            batch_activations.append(activations)

        IS_cam = torch.stack(batch_activations, dim=0)
    ...
    def get_seeds_from_cam(self, _mask_, category=None, max_num_sp=3, avg_sp_area=3):
        # segments_x = np.zeros(max_num_sp, dtype=np.int64)
        # segments_x = np.zeros((_mask.shape[0], max_num_sp), dtype=np.int64)
        # # segments_y = np.zeros(max_num_sp, dtype=np.int64)
        # segments_y = np.zeros((_mask.shape[0], max_num_sp), dtype=np.int64)


        # nz = np.nonzero(_mask)
        # present_labels = np.unique(nz[0])

        all_segments = np.zeros((_mask_.shape[0], max_num_sp, 2), dtype=np.int64)

        for _present_label in category:
            if _present_label == 0:
                break
            segments_x = np.zeros(max_num_sp, dtype=np.int64)
            # segments_y = np.zeros(max_num_sp, dtype=np.int64)
            segments_y = np.zeros(max_num_sp, dtype=np.int64)
            nz = np.nonzero(_mask_[_present_label])
            ...
            if len(nz[0]) != 0:
                ...
                m_np_roi = np.copy(_mask_[_present_label])[p[0]:pend[0] + 1, p[1]:pend[1] + 1]
            ...
            if (num_sp != 0) and (num_sp != 1):
                for i in range(num_sp):
                    ...
                    weighted_dtrans = distance_transform_edt(m_np_roi)
            ...
            all_segments[_present_label] = segments
